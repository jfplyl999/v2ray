...
modify_kcptun_port(){
        #修改kcp与v2ray连接端口
		sed -i "/\"target\"/c  \    \"target\":127.0.0.1:${port}," ${kcptun_server_conf}
    judge "kcptunport 修改"
}
modify_kcptun_port_client(){
        #修改kcp与v2ray连接端口
		sed -i "/\"remoteaddr\"/c  \    \"remoteaddr\":127.0.0.1:${port}," ${kcptun_clinet_conf}
    judge "kcptunport 修改"
}
modify_UUID(){
...
#    fi
    ...
    wget -N --no-check-certificate -nc http://nginx.org/download/nginx-${nginx_version}.tar.gz -P ${nginx_openssl_src}
    judge "Nginx 下载"
    wget -N --no-check-certificate -nc https://www.openssl.org/source/openssl-${openssl_version}.tar.gz -P ${nginx_openssl_src}
    judge "openssl 下载"
...
udp2raw_server_install(){
    ...
    wget -N --no-check-certificate https://github.com/jfplyl999/v2ray/raw/master/udp2raw_amd64 -O udp2raw_amd64 && wget -N --no-check-certificate https://raw.githubusercontent.com/jfplyl999/v2ray/master/run.sh -O run.sh && wget -N --no-check-certificate https://raw.githubusercontent.com/jfplyl999/v2ray/master/kcprestart.sh -O kcprestart.sh && chmod +x udp2raw_amd64 run.sh kcprestart.sh
...
kcptun_client_install(){
    ...
    wget -N --no-check-certificate https://raw.githubusercontent.com/jfplyl999/v2ray/master/kcptun_celient.json -O kcptun_client.json
...
kcptun_udp2raw_client_restart(){
    ...
    ps -ef|grep udp2raw|grep -v grep|cut -c 9-15|xargs kill -9
    sleep 3
    nohup ./run.sh ./udp2raw_amd64 -c -r ${bk_ip_pf}:8088 -10.0.0.0:9090 --raw-mode faketcp --cipher-mode none -a -k "atrandys" >udp2raw.log 2>&1 &
    nohup ./run.sh ./client_linux_amd64 -c ./kcptun_client.json >kcptun.log 2>&1 &
    sleep 3
    ps -ef|grep kcptun && ps -ef|grep udp2raw
}
...
menu(){
	...
	echo -e "${Green}24.${Font} 变更 kcp对接的服务器ip"
